...
export const ProjectCard: React.FC<ProjectCardProps> = ({ 
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          ...
          <span className={`text-[11px] font-medium px-2.5 py-0.5 rounded-full border ${statusInfo.bg} ${statusInfo.text} ${statusInfo.border} flex items-center gap-1`}>
            {project.status === 'completed' && (
              isArchived ? <Archive className="w-3 h-3 text-emerald-400" /> : <CheckCircle2 className="w-3 h-3 text-emerald-400" />
            )}
        ...
        <div className="bg-slate-900/80 rounded-xl p-3.5 border border-white/5 mb-3 space-y-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className={`font-extrabold text-sm ${project.status === 'cancelled' ? 'text-slate-400' : 'text-white'}`}>
              {formatCurrency(total, project.currency)}
              {project.status === 'cancelled' && <span className="text-[10px] font-normal text-zinc-400 ml-1">(сброшено)</span>}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 pt-1.5 text-xs">
            ...
            <div className={`p-2 rounded-lg border ${owed > 0 ? 'bg-amber-950/30 border-amber-500/20' : 'bg-slate-900 border-white/5'}`}>
              <span className="text-[10px] text-slate-400 font-semibold block flex items-center gap-1">
                {project.status === 'cancelled' ? (
                  <span className="text-zinc-400">Долг (сброшен):</span>
                ) : owed > 0 ? (
                  <><AlertCircle className="w-2.5 h-2.5 text-amber-400" /> Остаток (долг):</>
                ) : (
                  <><CheckCircle2 className="w-2.5 h-2.5 text-emerald-400" /> Статус оплаты:</>
                )}
              </span>
              <span className={`font-bold ${project.status === 'cancelled' ? 'text-zinc-400' : owed > 0 ? 'text-amber-400' : 'text-emerald-400'}`}>
                {project.status === 'cancelled' ? '0 ' + project.currency : owed > 0 ? formatCurrency(owed, project.currency) : 'Всё оплачено ✓'}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-white/5 text-xs text-slate-400 group-hover:text-white gap-2 flex-wrap">
        {isArchived && onRestoreProject ? (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onRestoreProject();
            }}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-300 border border-emerald-500/30 transition-all text-xs font-semibold"
            title="Вернуть проект в активную разработку"
          >
            <RotateCcw className="w-3.5 h-3.5" />
            <span>{project.status === 'cancelled' ? 'Восстановить проект' : 'Вернуть в работу'}</span>
          </button>
        ) : (
        ...
        <button 
          ...
          className="flex items-center gap-1 text-xs font-semibold text-indigo-400 hover:text-indigo-300 transition-colors group/btn ml-auto"
        >
